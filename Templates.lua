...
local function _AddQuestRewardsToTooltip(tooltip, questID, style)
	...
	if not style.atLeastShowAzerite then
		...
		local numAddedQuestCurrencies, usingCurrencyContainer = QuestUtils_AddQuestCurrencyRewardsToTooltip(questID, tooltip,
			itemToolTip);
	...
	if numQuestRewards > 0 and (not style.prioritizeCurrencyOverItem or numCurrencyRewards == 0) then
		if style.fullItemDescription then
			...
			local text;
			if (numItems > 1) then
				text = string.format(BONUS_OBJECTIVE_REWARD_WITH_COUNT_FORMAT, texture,
					HIGHLIGHT_FONT_COLOR:WrapTextInColorCode(numItems), name);
...
function WQT_Utils:GetQuestRewardIcon(questID)
	...
	if (currencyId) then
		...
		texture = select(2,
			CurrencyContainerUtil.GetCurrencyContainerInfo(currencyId, amount, currencyInfo.name, currencyInfo.iconFileID,
