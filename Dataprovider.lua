		...
		if (GetNumQuestLogRewards(self.questId) > 0) then
			...
			if (rewardId) then
				...
				if (C_Soulbinds.IsItemConduitByItemInfo(rewardId)) then
					...
					self:AddReward(WQT_REWARDTYPE.conduit, ilvl, texture, quality, WQT_Utils:GetColor(_V["COLOR_IDS"].rewardRelic),
						rewardId, false, subType);
				...
				elseif (C_Item.IsAnimaItemByID(rewardId)) then
					...
					self:AddReward(WQT_REWARDTYPE.anima, numItems * value, texture, quality,
						WQT_Utils:GetColor(_V["COLOR_IDS"].rewardAnima), rewardId);
				else
					-- Normal items
