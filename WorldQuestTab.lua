...
function WQT_RewardDisplayMixin:AddRewardByInfo(rewardInfo, warmodeBonus)
	...
	self:AddReward(rewardInfo.type, rewardInfo.texture, rewardInfo.quality, rewardInfo.amount, rewardInfo.textColor,
		rewardInfo.canUpgrade, warmodeBonus);
end
...
function WQT_QuestCounterMixin:InfoOnEnter(frame)
	...
	GameTooltip:AddDoubleLine("API - Addon = Displayed",
		("|cFFFFFFFF%d - %d = %d|r"):format(questCount, #self.falseCounted, self.numQuests), 1, 1, 1, 1, 1, 1, true);
	...
	for k, i in ipairs(self.falseCounted) do
		...
		GameTooltip:AddDoubleLine(string.format("%s (%s)", info.title, info.questID), tagInfo and tagInfo.tagName or "No tag",
			1, 1, 1, 1, 1, 1, true);
	end
	...
	if (fileDataID and fileDataID > 0) or (atlasID) then
		...
		if (atlasID) then
			...
			if textureX > 0 and textureY > 0 then
				...
				WQT_MapZoneHightlight.Texture:SetPoint("TOPLEFT", WQT_MapZoneHightlight:GetParent(), "TOPLEFT", scrollChildX,
					scrollChildY);
				WQT_MapZoneHightlight:Show();
			end
		end
